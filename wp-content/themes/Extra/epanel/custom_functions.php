	...
	function elegant_titles_filter( $custom_title ) {
		...
		if ( is_category() || is_archive() || is_search() || is_404() ) {
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				$page_title = sprintf( esc_html__( 'Seus resultados de busca para "%s"', $themename ), esc_attr( get_search_query() ) );
			} else if ( is_404() ) {
				$page_title = esc_html__( '404 Não encontrado', $themename );
			}
		...
		if ( $seo_index_description == 'on' ) {
			...
			if ( is_search() && ! $description_added ) {
				$description_text = $is_pre_4_4 ? wp_title( '', false, '' ) : sprintf(
					esc_html__( 'Resultados de busca para: %s', $themename ),
					get_search_query()
				);
